...
* Alt.prototype.dispatch -> Alt.prototype.store.dispatch

The method's signature has also changed:
...
* No warnings when attempting to add an unnamed branch. Alt now throws.

```js
alt.addBranch(new NamedBranch()) // <-- must have a name configured in this.namespace
```

* Actions are now purely FSA, `id`, `data`, `name` keys have been removed.

* Action dispatchers are not available on `alt.actions`.

* `createActions` no longer creates a unique namespace for each action dispatchers object passed in. It is up to the developer to provide one.

* Constant form for action dispatchers are no longer available: eg `foo.FOO_BAR === undefined`

* Removed config from branches. Can no longer be configured:

- ~~getState~~
- ~~setState~~
- ~~transform~~

* Dropped official support for creating branches using a POJO.

* Removes a few branch instance variables and methods:

- ~~branch.dispatchToken~~
- ~~branch.waitFor()~~
- ~~branch.listen()~~
- ~~branch.unlisten()~~

* Branches no longer auto-emit error events.

* takeSnapshot -> serialize

The method's signature also changed to:

```diff
  - takeSnapshot(...storeNamesToSerialize)
  + serialize()
```

* bootstrap -> load

* Removed `Alt.prototype.rollback()`
...
The term `action` refers to the object that is being dispatched. In Alt@0.x this term was overloaded to also refer to the functions that dispatched the action. Alt@2 chooses to call these what they really are dispatchers of an action.

Stores -> Branches

Alt borrowed the term "Store" from Flux which meant to be the place that would house your data and where you'd subscribe to for state change events. Under teh hood, the state was always kept a under single state tree, thus, these stores are really branches of that larger tree. This feature is what enabled things like bootstrap, and flush which were core components of making Alt work end-to-end from server to browser. Alt@2 is calling these individual data warehouses "branches".

Dispatcher -> Store

The "Dispatcher" was replaced by a single Store which takes care of dispatching an action throughout your entire state tree, it also holds all of the application state.

* Instance variables removed
  * `alt._actionsRegistry`
  * `alt._lastSnapshot`
  * `alt.batchingFunction`
  * `alt.trapAsync`

* Dropped support for configuring Alt's internals:
  - ~~serialize~~
  - ~~deserialize~~
  - ~~dispatcher~~

* Branch methods renamed or removed.
  - bindActions -> respondToAll
  - bindListeners -> respondTo
  - bindAction -> setActionResponder
  - ~~exportAsync~~
  - ~~registerAsync~~
  - ~~emitChange~~

* Action Dispatcher's `defer` has been removed.

* Action dispatchers are no longer stored in an Alt instance variable.
...
#### AltZero Breaking Changes

* Sources can no longer be functions that return an Object.
