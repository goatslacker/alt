...
export default {
  'make final store'() {
    ...
    FinalStore.listen(() => {
      assert(++finalStoreCalls === 1, 'final store was called only once')
    ...
    store2.listen(() => {
      assert(++store2Calls === 1, 'store 2 listen was only called once')
      assert(store2.getState().value === 2, 'store2 state was updated successfully')
    })
